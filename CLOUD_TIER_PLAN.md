...
#### 1.1 Payment System (Nosflare Pay-to-Relay)

- [x] Use built-in Nosflare pay-to-relay with nostr-zap.js
- [x] Configure relay operator npub for receiving payments
- [x] Set RELAY_ACCESS_PRICE_SATS = 12121
- [ ] Deploy NWC (Nostr Wallet Connect) for payment notifications

**Technical Details**:

```typescript
// relay/nosflare/src/config.ts (generated by deploy.sh)
...
**Payment Flow**:
1. User visits relay landing page
2. User clicks "Pay 12,121 sats" button (triggers nostr-zap)
3. User pays via Lightning (NWC, extension, or direct)
4. Relay detects payment via zap notification
5. Relay records pubkey in paid_pubkeys table
6. Connection URL revealed to user
...
│  3. Nostr-zap triggers Lightning invoice (12,121 sats)       │
│  4. Nostr-zap shows QR code / NWC / extension prompt        │
│  5. User pays via Lightning wallet                          │
│  6. Zap receipt detected by relay                           │
│  7. Relay records pubkey in paid_pubkeys table              │
│  8. Landing page reveals wss:// connection URL              │
│  9. User can now publish events to managed relay            │
│                                                             │
└────────────────────────────────────────────────────────────┘
```
...
**Yes** - 12,121 sats one-time covers relay infrastructure costs. Provides clear upgrade
path from free tier. One-time payment simplifies user onboarding.
