...
const envSlug = $derived(page.params.env);
...
// Find the selected environment from URL param
const selectedEnv = $derived(
	project?.environments.find((e) => e.slug === envSlug) ?? project?.environments[0],
);

// Redirect to valid environment if URL env doesn't exist
$effect(() => {
	if (project && project.environments.length > 0) {
		const envExists = project.environments.some((e) => e.slug === envSlug);
		if (!envExists) {
...
// Navigate to a different environment via URL
function selectEnvironment(env: Environment) {
	goto(`/admin/projects/${projectSlug}/${env.slug}`);
}
...
<svelte:head>
	<title>{project?.displayName ?? 'Project'} / {selectedEnv?.name ?? 'Environment'} - Redshift Admin</title>
</svelte:head>
