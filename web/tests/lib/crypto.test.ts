...
/**
 * Cryptographic Core Tests for Web - Using @redshift/crypto shared package
 *
 * Tests for NIP-44 encryption and NIP-59 Gift Wrap implementation.
 */
...
describe('NIP-59 Gift Wrap', () => {
	...
	it('handles complex nested objects as JSON strings', () => {
		const secrets: SecretBundle = {
	...
	it('outer event uses ephemeral pubkey (not owner pubkey)', () => {
		const secrets: SecretBundle = { KEY: 'value' };
		const dTag = 'proj|env';
		const ownerPubkey = getPublicKey(testPrivateKey);
...
describe('Secret Bundle Format', () => {
	...
	it('preserves string values exactly', () => {
		const secrets: SecretBundle = {
			...
			WITH_SPECIAL: 'key=value&foo=bar',
			MULTILINE: 'line1\nline2\nline3',
			EMPTY: '',
		};
		const dTag = 'test|env';
