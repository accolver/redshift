...
import { deleteTagFilter, getTagFilter, getTagFilterEntries, setTagFilter } from './types';
...
async function initializeDatabase(db: D1Database): Promise<void> {
	try {
		...
		const initCheck = await session
			.prepare("SELECT value FROM system_config WHERE key = 'db_initialized' LIMIT 1")
			.first()
			.catch(() => null);
    ...
    `)
		...
		await session
			.prepare("INSERT OR REPLACE INTO system_config (key, value) VALUES ('db_initialized', '1')")
			.run();
      ...
      `)
		...
		await session
			.prepare("INSERT OR REPLACE INTO system_config (key, value) VALUES ('schema_version', '6')")
			.run();
	...
	try {
		...
		if (shouldCheckForDuplicates(event.kind)) {
			...
			const duplicateContent = enableGlobalDuplicateCheck
				? await session
						.prepare('SELECT event_id FROM content_hashes WHERE hash = ? LIMIT 1')
						.bind(contentHash)
						.first()
				: await session
						.prepare('SELECT event_id FROM content_hashes WHERE hash = ? AND pubkey = ? LIMIT 1')
	...
	for (const eventId of deletedEventIds) {
		try {
			...
			const existing = await session
				.prepare('SELECT pubkey FROM events WHERE id = ? LIMIT 1')
				.bind(eventId)
				.first();
...
async function queryDatabaseChunked(
	...
	const processStringChunks = async (filterType: 'ids' | 'authors' | string, values: string[]) => {
		...
		for (const chunk of chunks) {
			const chunkFilter: NostrFilter = { ...baseFilter };
	...
	for (const [tagKey] of Object.entries(needsChunking.tags)) {
		// tagKey is like '#p', '#e', etc. - extract the tag name after '#'
	...
	try {
		...
		for (const filter of filters) {
			...
			const needsChunking =
				...
				getTagFilterEntries(filter).some(([, values]) => values.length > CHUNK_SIZE);
    ...
    <div class="container">
        ...
        <div class="benefits">
            <div class="benefit-card">
                ...
                <span class="benefit-text">Global edge network</span>
            </div>
            <div class="benefit-card">
                <svg class="benefit-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <span class="benefit-text">Encrypted storage</span>
            </div>
            <div class="benefit-card">
                <svg class="benefit-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
async function handlePaymentNotification(request: Request, env: Env): Promise<Response> {
	...
	try {
		...
		// Parse request body for zap receipt
		let body: { zapReceipt?: NostrEvent };
		try {
			body = (await request.json()) as { zapReceipt?: NostrEvent };
		} catch {
		...
		// 1. Verify it's a kind 9735 zap receipt
		...
		// 2. Verify the zap receipt has required fields
		...
		// 3. Verify the zap receipt signature
		...
		// 4. Verify the zap receipt's p tag references the relay's pubkey (payment was TO the relay)
		const pTag = zapReceipt.tags.find((t: string[]) => t[0] === 'p');
		if (!pTag || pTag[1] !== relayInfo.pubkey) {
			return new Response(
				JSON.stringify({
		...
		// 5. Verify the zap receipt references the correct npub via the description tag
		...
		try {
			zapRequest = JSON.parse(descriptionTag[1]) as { pubkey?: string; tags?: string[][] };
		} catch {
			return new Response(
		...
		// The zap request's pubkey is the sender (the npub who paid)
		...
		// All checks passed - save the pubkey as paid
...
async function getDatabaseSizeBytes(session: D1DatabaseSession): Promise<number> {
	try {
		const pageCountResult = (await session.prepare('PRAGMA page_count').first()) as {
			page_count: number;
		} | null;
		const pageSizeResult = (await session.prepare('PRAGMA page_size').first()) as {
			page_size: number;
		} | null;
...
async function pruneOldEvents(
	...
	const protectedKindsClause =
		protectedKindsArray.length > 0 ? `AND kind NOT IN (${protectedKindsArray.join(',')})` : '';
