...
> **Status: Legacy non-normative PRD.** Current shipped truth is defined by
> [`openspec/specs/`](openspec/specs/) and named release evidence, including
> [`piolium/authenticated-secret-history-evidence.md`](piolium/authenticated-secret-history-evidence.md).
> Cloud, Teams, Enterprise, pricing, retention, and SLA material in this document
> is historical research unless a current approved OpenSpec explicitly says otherwise.
...
- **Deletion:**
  - Redshift secret deletion publishes a newer authenticated empty bundle as a
    logical tombstone for the same d-tag.
  - NIP-09 Kind 5 deletion requests cannot authorize deletion of Gift Wraps
    signed by ephemeral outer keys. Redshift uses NIP-09 only for events owned
    by the deleting identity, such as project metadata. Neither mechanism
    guarantees erasure from relays, caches, exports, or backups.
...
5. **Event Creation with EventFactory**
   - Use `EventFactory` from `applesauce-core/event-factory` to create events
   - Use blueprints for consistent event structure
   - Sign events with the authenticated signer (NIP-07, NIP-46, or local key)
